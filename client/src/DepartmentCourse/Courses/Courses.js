...
const Courses = ({ filter,expanded,setDepartmentZH_TW}) => {
  ...
  setDepartmentZH_TW(courses[0]?.departmentZH_TW)
  useEffect(() => {    
...
const courseFilter = (collections,filters) => {
  const {  searchTerm, searchTerm2, orderMode, subType, typeOfCredit } = filters;
  ...
  if(typeOfCredit!==''){
    collections = collections.filter(course => course.typeOfCredit === typeOfCredit);
  }
