    ...
    isLoadingCourse ? <CircularProgress /> : (
      <Grid className={classes.container} container alignItems="stretch" spacing={2}>
        {filteredCourses?.map((course) => (
          <Grid key={course._id} item xs={12} sm={6} md={4} lg={4}>
...
const courseFilter = (collections,filters) => {
  const {  searchTerm, searchTerm2, orderMode, type, subType } = filters;
  ...
  if(type!==''){
    collections = collections.filter(course => course.type === type);
  }
